...
class BiChannelAttention(nn.Module):
    def __init__(self, heads: int, utterance_dim: int, time_dim: int, attention_type: str, local_window_num: int, dropout_rate: float):
        ...
        # FFN
        # hidden_size = utterance_dim + time_dim * heads
...
class SingleHeadBiChannelAttention(nn.Module):
    def __init__(self, head_utterance_dim: int, head_time_dim: int, attention_type: str, local_window_num: int, dropout_rate: float, position_parameter: nn.Parameter):
    ...
    def forward(self, t: int, content_t_chunk: torch.Tensor, time_mask: torch.Tensor, cache_t_chunk: torch.Tensor, speakers: torch.Tensor) -> torch.Tensor:
        ...
        attention_map /= (self.head_utterance_dim + self.head_time_dim) ** 0.5
        ...
        # attention map * v
        output = torch.bmm(attention_map.unsqueeze(1), v).squeeze(1)   # (B, D/head + time_dim)
